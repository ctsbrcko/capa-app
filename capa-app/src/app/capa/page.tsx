...
function isCapaRowOverdue(
  record: {
    due_date?: string | null;
    status_id: string;
  },
  statusCodeById: Map<string, string>,
): boolean {
  const due = record.due_date;
  const status_code = (statusCodeById.get(record.status_id) ?? "").toUpperCase();
  return Boolean(
    due &&
          ...
          <div className="mt-3 overflow-hidden rounded-lg border border-slate-800">
            <table className="min-w-full divide-y divide-slate-800 text-sm">
              ...
              <tbody className="divide-y divide-slate-800 bg-slate-950/40">
                ...
                {records.map((record) => {
                  const isOverdue = isCapaRowOverdue(record, statusCodeById);
                  return (
                  <tr
                    key={record.id}
                    className={`${isOverdue ? "bg-red-900/40 " : ""}hover:bg-slate-900/80`}
                  >
